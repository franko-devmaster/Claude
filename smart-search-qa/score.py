...
def ndcg_at_k(titles, expectations, k=5):
    gains = [1.0 if matches(t, expectations) else 0.0 for t in titles[:k]]
    dcg = sum(g / math.log2(i + 2) for i, g in enumerate(gains))
    ideal = sum(1.0 / math.log2(i + 2) for i in range(min(len(expectations) or 1, k)))
...
def main():
    ...
    lines = [
        ...
        "",
        "Cible: %s · corpus v%s · %d cas · %d appels"
        % (corpus.get("target"), corpus.get("corpus_version"), len(rows), len(records)),
        "",
        "## Synthese par categorie",
        "",
        "| Categorie | Cas | Hit@1 | Hit@3 | MRR | nDCG@5 | Stab. top-3 | Stab. rang 1 | Latence med. | Sans resultat |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for cat in sorted({r["category"] for r in rows}):
        subset = [r for r in rows if r["category"] == cat]
        latency = statistics.median(r["latency_ms"] for r in subset)
        lines.append("| %s — %s | %d | %s | %s | %s | %s | %s | %s | %.0f ms | %s |" % (
            cat, CATEGORY_LABELS.get(cat, ""), len(subset),
            pct(aggregate(subset, "hit1")), pct(aggregate(subset, "hit3")),
            ...
            pct(aggregate(subset, "stability")), pct(aggregate(subset, "top1_agreement")),
            latency, pct(aggregate(subset, "empty_rate")),
        ))
